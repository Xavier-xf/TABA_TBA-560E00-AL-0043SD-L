...
static struct option option_long[ ] = {
    ...
    { "reset-screen"      , no_argument       , NULL , 'C' } ,                  //"重置屏幕参数" ,
...
int test_tde( void )                                                            //tde图形操作测试
{
    ...
    if ( ( i_filesize_src != i_dmasize_src ) || ( i_filesize_bg != i_dmasize_bg ) ) {
        ak_print_error( MODULE_ID_APP, "FILE SIZE NOT FIT FORMAT. i_filesize_src= %d i_dmasize_src= %d i_filesize_bg= %d i_dmasize_bg= %d\n",
