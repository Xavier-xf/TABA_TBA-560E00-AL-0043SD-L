...
static int flash_jedec_id_check(struct spi_slave *slave)
{
    ...
    // if ((id[2] != 0x18) || (id[1] != 0x40) || (id[0] != 0x20))           //XM25QH128C
    if ((id[2] != 0x18) || (id[1] != 0x20) || (id[0] != 0xc2))       //MX25L12833F
    {
        return -1;
    }
    return 0;
...
static int flash_unique_id_read(struct spi_slave *slave, unsigned char *id, int size)
{
    ...
    char SCUR_VAL = 0X0;
    char TEMP_SCUR_VAL = 0X00;

    ret = spi_xfer(slave, 8, &WREN, NULL, SPI_XFER_BEGIN | SPI_XFER_END);
    if (ret){
        printf("spi cmd %#x send failed\n", WREN);
        return ret;
    }

    ret = spi_xfer(slave, 8, &RDSCUR, NULL, SPI_XFER_BEGIN);
    if (ret){
        printf("spi cmd %#x send failed\n", RDSCUR);
        return ret;
    }

    ret = spi_xfer(slave, 8, NULL, &SCUR_VAL, SPI_XFER_BEGIN | SPI_XFER_END);
    if (ret)
    {
        printf("spi read scur failed\n");
        return ret;
    }


    printf("SCUR_VAL is 0x%x\r\n",SCUR_VAL);

    SCUR_VAL &= ~(0X01);



    ret = spi_xfer(slave, 8, &WRSCUR, NULL, SPI_XFER_BEGIN | SPI_XFER_END);
    if (ret){
        printf("spi cmd %#x send failed\n", WRSCUR);
        return ret;
    }

    ret = spi_xfer(slave, 8, &SCUR_VAL, NULL, SPI_XFER_BEGIN | SPI_XFER_END);
    if (ret)
    {
        printf("spi read scur failed\n");
        return ret;
    ...
    ret = spi_xfer(slave, 8, &ENSO, NULL, SPI_XFER_BEGIN | SPI_XFER_END);
    if (ret){
        printf("spi cmd %#x send failed\n", ENSO);
        return ret;
    }

    unsigned char addres[] = {0X03, 0x00, 0x02, 0x00};
    ...
    ret = spi_xfer(slave, 8 * size, NULL, id, SPI_XFER_BEGIN | SPI_XFER_END);
    if (ret)
    {
        printf("spi read jedec id failed\n");
        return ret;
    ...
    ret = spi_xfer(slave, 8, &EXSO, NULL, SPI_XFER_BEGIN | SPI_XFER_END);
    if (ret){
        printf("spi cmd %#x send failed\n", EXSO);
        return ret;
    }
    ...
    #else
    int ret = 0, i = 0;
    unsigned char cmd[5] = {0x5A, 0, 0X01, 0XE0, 0};
...
static int security_register_read(struct spi_slave *slave, unsigned char *reg, int size)
{
    ...
    unsigned char addres[] = {0X03, 0x00, 0x00, 0x00};
    ...
    ret = spi_xfer(slave, 8 * size, NULL, reg, SPI_XFER_BEGIN | SPI_XFER_END);
    if (ret)
    {
        printf("spi read jedec id failed\n");
        return ret;
...
static int do_uuid_check(cmd_tbl_t *cmdtp, int flag, int argc, char *const argv[])
{
    ...
    {
        // printf("md5_data:%02x----------------security_register_data:%02x\n", md5_data[i],security_register_data[i]);
