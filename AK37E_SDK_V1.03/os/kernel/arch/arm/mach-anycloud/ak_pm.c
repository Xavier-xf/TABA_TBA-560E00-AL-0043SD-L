...
static int ak_pm_probe(struct platform_device *pdev)
{
	...
	for (index = 0; index < number; index++) {
		...
		if (ret < 0){
			printk(KERN_ERR"%s, line:%d, err: get no wakeup-mode!\n", __func__,__LINE__);
			ret = -1;
			return ret;
		}

		if (ak_pm_info.wakeup_mode[index] == 0x1){
			ak_pm_info.wakeup_gpio_index = of_get_named_gpio(node, "wakeup-gpio", 0);
			if (ak_pm_info.wakeup_gpio_index < 0){/* gpio wakeup */
				ak_pm_info.wakeup_gpio_index = 0x0;
				printk(KERN_ERR"%s, line:%d, err: get no wakeup_gpio_index!\n", __func__,__LINE__);
			}

			ret = of_property_read_u32(node, "wakeup-gpio-edge",&ak_pm_info.wakeup_gpio_edge);
			if (ret < 0){
				ak_pm_info.wakeup_gpio_edge = 0x0;
				printk(KERN_ERR"%s, line:%d, err: get no wakeup_gpio_edge!\n", __func__,__LINE__);
			}
		}
