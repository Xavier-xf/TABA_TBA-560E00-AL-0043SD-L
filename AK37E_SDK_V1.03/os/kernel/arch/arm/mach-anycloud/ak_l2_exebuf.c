...
static int ak_pm_gpio_wakeup_set(int pm_wakeup_gpio, int pm_wakeup_gpio_edge)
{
	int i = 0;
	int gpio_regbit = 0;
	unsigned long value;
	...
	for (i=0; i<sizeof(wakeup_all_gpio); i++){
		if(pm_wakeup_gpio == wakeup_all_gpio[i]){
			//printk(KERN_ERR"%s , line:%d , pm wakeup gpio ok:%d\n", __func__, __LINE__, pm_wakeup_gpio);
			gpio_regbit = i;
			break;
		}
	}
	...
	if (i == sizeof(wakeup_all_gpio)){
		printk(KERN_ERR"%s , line:%d , pm wakeup gpio %d set err!\n", __func__, __LINE__, pm_wakeup_gpio);
		ret = -1;
		return ret;
	}

	/* set GPIO sharepin as gpio ,enable pu, and input direction */
	if (gpio_direction_input(pm_wakeup_gpio)){
	...
	for (index=0; index<ak_pm_info.wakeup_mode_cnt; index++){
		switch(ak_pm_info.wakeup_mode[index]){
			case AK_PM_WAKEUP_GPIO_MODE:
				ret = ak_pm_gpio_wakeup_set(ak_pm_info.wakeup_gpio_index, ak_pm_info.wakeup_gpio_edge);
				if (ret < 0){
