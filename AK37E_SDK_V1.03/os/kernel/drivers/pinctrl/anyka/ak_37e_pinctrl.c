...
static int ak_gpio_suspend(void)
{
	...
	*/
    ...
    for (i = 0; i < ARRAY_SIZE(ak_sharepin); i++) {
        if ((i != reset_pin) && (i != power_pin) && (i != backlight_pin) && (i != ak_wakeup_gpio)) {
		...
		if (wakeup_mode_cnt > 0) {
			for (i = 0; i < wakeup_mode_cnt; i++) {
				...
				if (ret) {
					...
					ak_wakeup_gpio = of_get_named_gpio(np, "wakeup-gpio", 0);
					break;
				}
			}
		} else {
