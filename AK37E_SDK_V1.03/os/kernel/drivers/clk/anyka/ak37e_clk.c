...
#define CLOCK_MAC1_OPCLK_EN_CFG_MASK				(0x01)
...
#define CLOCK_MAC0_OPCLK_EN_CFG_MASK				(0x01)
...
#define CLOCK_SPI_HSCLK_EN_CFG_MASK					(0x01)
...
#define CLOCK_PDM_HS_CLK_EN_CFG_MASK				(0x01)
...
* CLOCK_LCD_PCLK_CTRL
*/
#define CLOCK_LCD_PCLK_EN_CFG_MASK					(0x01)
...
static unsigned long audio_pll[] = {204800000, 282240000, 168960000, 204800000, 282240000, 184320000,
	204800000, 282240000, 184320000, 293538461, 245760000};
static unsigned char audio_pll_m[] = {14, 24, 24, 14, 24, 24,
	14, 24, 24, 25, 24}; /*width:5bits*/
static unsigned short audio_pll_n[] = {63, 146, 87, 63, 146, 95,
	63, 146, 95, 158, 127}; /*width:9bits*/

static unsigned short sddac_DIV[] = {100, 100, 55, 50, 50, 30,
	25, 25, 15, 13, 10};
static unsigned short sdadc_DIV[] = {100, 100, 55, 50, 50, 30,
	25, 25, 15, 13, 10};
	...
	unsigned long rate, unsigned long parent_rate)
{
	...
	*/
	switch (fixed_clk->id) {
		case AUDIO_PLL_CLK:
			now_rate = ak_get_audio_pll_freq(fixed_clk->reg);
			if (now_rate != rate)
				ak_set_audio_pll_freq(fixed_clk->reg, rate);
			break;
		default:
			break;
	}
...
* @rate -> pdm_sr
*/
static unsigned int pdm_calc_div_ratio(unsigned long pll, unsigned int rate, unsigned int *div,
				unsigned int *decimation_ratio_cfg_val)
{
	...
	for (i = 0; i < sizeof(decimation_ratio_list)/sizeof(unsigned int); i++) {
		decimation_ratio_sel = sizeof(decimation_ratio_list)/sizeof(unsigned int) - i;
		...
		*div = pll / (rate * decimation_ratio * 2);
		pr_info("%s pll#%lu pdm_i2sm_clk %lu pdm_clk %lu pdm_sr %lu\n", __func__, pll, pll/(*div),
			pll/(*div * 2), pll/(*div * 2 * decimation_ratio_list[decimation_ratio_sel] * 4));

		break;
	}

	if (i == sizeof(decimation_ratio_list)/sizeof(unsigned int)) {
		printk(KERN_ERR"%s invaild decimation_ratio\n", __func__);
		return -EINVAL;
	}
		...
		unsigned long rate, unsigned long parent_rate)
{
	...
	if (rate == 0) {
		reg_val = __raw_readl(reg + CLOCK_AUDIO_PLL_CTRL);
		reg_val &= ~(CLOCK_PDM_I2SM_CLK_EN_CFG_MASK << CLOCK_PDM_I2SM_CLK_EN_CFG_SHIFT);
		__raw_writel(reg_val, (reg + CLOCK_AUDIO_PLL_CTRL));
		return 0;
	}
	...
	ak_adjust_clock_integer_divider_freq((reg + CLOCK_AUDIO_PLL_CTRL),
		CLOCK_PDM_I2SM_CLK_DIV_VLD_CFG_SHIFT, CLOCK_PDM_I2SM_CLK_EN_CFG_SHIFT, (div - 1),
	...
	reg_val &= ~(PDM_CTRL_DECIMATION_RATIO_CFG_MASK << PDM_CTRL_DECIMATION_RATIO_CFG_SHIFT);
	reg_val |= ((ratio & PDM_CTRL_DECIMATION_RATIO_CFG_MASK) << PDM_CTRL_DECIMATION_RATIO_CFG_SHIFT);
		...
		unsigned long rate, unsigned long parent_rate)
{
	...
	if (rate == 0) {
		...
		regval &= ~(CLOCK_PDM_HS_CLK_EN_CFG_MASK << CLOCK_PDM_HS_CLK_EN_CFG_SHIFT);
		__raw_writel(regval, (reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL));
		return 0;
	}
...
static int i2s0_mclk_sdadc_set_div(struct ak_factor_clk *factor_clk, int factor)
{
	unsigned int regval;
	void __iomem *reg = factor_clk->reg;
...
static int i2s0_b_lr_sdadc_set_div(struct ak_factor_clk *factor_clk, int factor)
{
	unsigned int regval;
	...
	switch (factor_clk->id) {
		case AUDIO_FACTOR_SDADC_HSCLK:
			if (rate == 0) {
				...
				regval &= ~(CLOCK_SDADC_HS_CLK_EN_CFG_MASK << CLOCK_SDADC_HS_CLK_EN_CFG_SHIFT);
		...
		case AUDIO_FACTOR_SDDAC_HSCLK:
			if (rate == 0) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_HS_CTRL);
				regval &= ~(CLOCK_SDDAC_HS_CLK_EN_CFG_MASK << CLOCK_SDDAC_HS_CLK_EN_CFG_SHIFT);
				__raw_writel(regval, reg + CLOCK_SD_ADC_DAC_HS_CTRL);
			} else {
		...
		case AUDIO_FACTOR_SDADC_CLK:
			if (rate == 0) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
				regval &= ~(CLOCK_SDADC_CLK_EN_CFG_MASK << CLOCK_SDADC_CLK_EN_CFG_SHIFT);
				__raw_writel(regval, reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
			} else {
		...
		case AUDIO_FACTOR_SDDAC_CLK:
			if (rate == 0) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
				regval &= ~(CLOCK_SDDAC_CLK_EN_CFG_MASK << CLOCK_SDDAC_CLK_EN_CFG_SHIFT);
				__raw_writel(regval, reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
			} else {
		...
		case AUDIO_FACTOR_PDM_I2SM_CLK:
			if (rate == 0) {
				regval = __raw_readl(reg + CLOCK_AUDIO_PLL_CTRL);
				regval &= ~(CLOCK_PDM_I2SM_CLK_EN_CFG_MASK << CLOCK_PDM_I2SM_CLK_EN_CFG_SHIFT);
				__raw_writel(regval, reg + CLOCK_AUDIO_PLL_CTRL);
			} else {
				pdm_set_div_ratio(factor_clk, rate, parent_rate);
			}
			break;
		case AUDIO_FACTOR_I2S0_MCLK:
			if (rate == 0) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
				regval &= ~(CLOCK_SDADC_CLK_EN_CFG_MASK << CLOCK_SDADC_CLK_EN_CFG_SHIFT);
				__raw_writel(regval, reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
			...
			break;
		case AUDIO_FACTOR_I2S0_B_LR_CLK:
			if (rate == 0) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
				regval &= ~(CLOCK_SDDAC_CLK_EN_CFG_MASK << CLOCK_SDDAC_CLK_EN_CFG_SHIFT);
				__raw_writel(regval, reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
		...
		unsigned long parent_rate)
{
	...
	if (!((regval_div >> CLOCK_SDADC_CLK_EN_CFG_SHIFT) & CLOCK_SDADC_CLK_EN_CFG_MASK))
		return 0;
		...
		unsigned long parent_rate)
{
	...
	regval_div = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
	if (!((regval_div >> CLOCK_SDDAC_CLK_EN_CFG_SHIFT) & CLOCK_SDDAC_CLK_EN_CFG_MASK))
		return 0;
		...
		unsigned long parent_rate)
{
	...
	if (!((regval_div >> CLOCK_SDADC_HS_CLK_EN_CFG_SHIFT) & CLOCK_SDADC_HS_CLK_EN_CFG_MASK))
		return 0;
		...
		unsigned long parent_rate)
{
	...
	regval_div = __raw_readl(reg + CLOCK_SD_ADC_DAC_HS_CTRL);
	if (!((regval_div >> CLOCK_SDDAC_HS_CLK_EN_CFG_SHIFT) & CLOCK_SDDAC_HS_CLK_EN_CFG_MASK))
		return 0;
		...
		unsigned long parent_rate)
{
	...
	if (!((regval >> CLOCK_PDM_I2SM_CLK_EN_CFG_SHIFT) & CLOCK_PDM_I2SM_CLK_EN_CFG_MASK))
		return 0;

	div = ((regval >> CLOCK_PDM_I2SM_CLK_DIV_NUM_CFG_SHIFT) & CLOCK_PDM_I2SM_CLK_DIV_NUM_CFG_MASK) + 1;

	regval = __raw_readl(AK_VA_SUBCTRL + PDM_CTRL_REG);
	ratio = (regval >> PDM_CTRL_DECIMATION_RATIO_CFG_SHIFT) & PDM_CTRL_DECIMATION_RATIO_CFG_MASK;
					...
					unsigned long parent_rate)
{
	...
	switch (factor_clk->id) {
		case AUDIO_FACTOR_SDADC_HSCLK:
			regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_HS_CTRL);
			if ((regval >> CLOCK_SDADC_HS_CLK_EN_CFG_SHIFT) & CLOCK_SDADC_HS_CLK_EN_CFG_MASK) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_HS_CTRL);
		...
		case AUDIO_FACTOR_SDDAC_HSCLK:
			regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_HS_CTRL);
			if ((regval >> CLOCK_SDDAC_HS_CLK_EN_CFG_SHIFT) & CLOCK_SDDAC_HS_CLK_EN_CFG_MASK) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_HS_CTRL);
		...
		case AUDIO_FACTOR_I2S0_MCLK:
			regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
			if ((regval >>CLOCK_SDADC_CLK_EN_CFG_SHIFT) & CLOCK_SDADC_CLK_EN_CFG_MASK) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
				factor = (regval >> CLOCK_SDADC_CLK_DIV_NUM_CFG_SHIFT) & CLOCK_SDADC_CLK_DIV_NUM_CFG_MASK;
				recalc_rate = parent_rate/(factor + 1);
			} else {
				recalc_rate = 0;
			}
		...
		case AUDIO_FACTOR_I2S0_B_LR_CLK:
			regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
			if ((regval >>CLOCK_SDDAC_CLK_EN_CFG_SHIFT) & CLOCK_SDDAC_CLK_EN_CFG_MASK) {
				regval = __raw_readl(reg + CLOCK_SD_ADC_DAC_AUDIO_CTRL);
				factor = (regval >> CLOCK_SDDAC_CLK_DIV_NUM_CFG_SHIFT) & CLOCK_SDDAC_CLK_DIV_NUM_CFG_MASK;
				recalc_rate = parent_rate/(factor + 1);
			} else {
				recalc_rate = 0;
			}
...
static void ak_factor_asic_clk_disable(struct clk_hw *hw)
{
	...
	switch (factor_clk->id) {
		...
		case ASIC_FACTOR_CSI1_SCLK:
			regval = __raw_readl(reg + CLOCK_SPI_HS_CTRL);
			regval &= ~(CLOCK_CSI1_SCLK_EN_CFG_MASK << CLOCK_CSI1_SCLK_EN_CFG_SHIFT);
			__raw_writel(regval, reg + CLOCK_SPI_HS_CTRL);
			break;
		case ASIC_FACTOR_MAC0_OPCLK:
			regval = __raw_readl(reg + CLOCK_MAC_OPCLK_CTRL);
			regval &= ~(CLOCK_MAC0_OPCLK_EN_CFG_MASK << CLOCK_MAC0_OPCLK_EN_CFG_SHIFT);
			__raw_writel(regval, reg + CLOCK_MAC_OPCLK_CTRL);
			break;
		case ASIC_FACTOR_MAC1_OPCLK:
			regval = __raw_readl(reg + CLOCK_MAC_OPCLK_CTRL);
			regval &= ~(CLOCK_MAC1_OPCLK_EN_CFG_MASK << CLOCK_MAC1_OPCLK_EN_CFG_SHIFT);
			__raw_writel(regval, reg + CLOCK_MAC_OPCLK_CTRL);
			break;
		case ASIC_FACTOR_LCD_PCLK:
			regval = __raw_readl(reg + CLOCK_LCD_PCLK_CTRL);
			regval &= ~(CLOCK_LCD_PCLK_EN_CFG_MASK << CLOCK_LCD_PCLK_EN_CFG_SHIFT);
			__raw_writel(regval, reg + CLOCK_LCD_PCLK_CTRL);
			break;
		case ASIC_FACTOR_SPI0_CLK:
			regval = __raw_readl(reg + CLOCK_SPI_HS_CTRL);
			regval &= ~(CLOCK_SPI_HSCLK_EN_CFG_MASK << CLOCK_SPI_HSCLK_EN_CFG_SHIFT);
			__raw_writel(regval, reg + CLOCK_SPI_HS_CTRL);
			break;
		default:
			pr_err("%s: unknow ID: %d\n", __func__, factor_clk->id);
...
static void ak_factor_audio_clk_disable(struct clk_hw *hw)
{
	struct ak_factor_clk *factor_clk = to_clk_ak_factor(hw);
	u32 regval;
	void __iomem *reg = factor_clk->reg;
...
const struct clk_ops ak_factor_audio_clk_ops = {
	...
	.disable	= ak_factor_audio_clk_disable,
};
...
static void ak_factor_sar_adc_clk_disable(struct clk_hw *hw)
{
	struct ak_factor_clk *factor_clk = to_clk_ak_factor(hw);
	void __iomem *reg = factor_clk->reg;
	u32 regval;
...
const struct clk_ops ak_factor_sar_adc_clk_ops = {
	.disable	= ak_factor_sar_adc_clk_disable,
		...
		const struct clk_ops *ops)
{
	...
	if (!strcmp(name, "pdm_i2sm")
		|| !strcmp(name, "i2s0_mclk")
		|| !strcmp(name, "i2s0_b_lr_clk"))
...
static void asic_pll_init(void __iomem *reg, u32 asic_pll, u32 div_od, u32 div_n)
{
	...
	*/
	...
	regval |= (1 << CLOCK_IMAGE_CAPTURE_CFG_SHIFT);
