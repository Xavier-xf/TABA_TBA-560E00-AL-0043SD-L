 ...
 * @mode[IN]  :  0 - 与ak_vo_refresh_screen()保持一致.( As ak_vo_refresh_screen() function )
                 ...
                 可选值:
                 0x0 - 同ak_vo_refresh_screen
                 0x1 - 不进行tde预处理,单纯等待lcdc可用
                 0x2 - 判断lcd控制器是否可用，如果处于非可用状态，则对图层进行遍历，
                       找到一个和屏幕同等尺寸的图层，将其以上的图层blit到该图层，
                       然后，再将处理后的图层拷贝到FB内存上
                 0x3 - 判断lcd控制器是否可用，如果处于非可用状态，则对图层进行遍历，
                       找到一个和屏幕同等尺寸的图层，将其以上的图层blit到该图层，
                       等lcd控制器可用后，再将再将处理后的图层拷贝到FB内存上
                 0x4 - 判断lcd控制器是否可用，如果处于非可用状态，则创建一个同fb缓冲区大小的临时图层
                       将全部图层全部BLIT上去后，再将临时图层拷贝到FB内存上，
                 0x5 - 判断lcd控制器是否可用，如果处于非可用状态，则创建一个同fb缓冲区大小的临时图层
