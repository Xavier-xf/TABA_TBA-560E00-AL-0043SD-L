...
wchar_t* convertArabic(wchar_t* normal,wchar_t* conver_str)
{
    int len = wcslen(normal);
    CharRep crep ;
    CombCharRep combcrep;
    unsigned short  shaped[256]= {0};// = (unsigned short *) malloc(sizeof(unsigned short) * (len + 1));
    int writeCount = 0;
    int i, d;
  //  wchar_t* toReturn = NULL;
   ...
   // if (!shaped)
    ...
    {
        conver_str[d] = shaped[writeCount - d - 1];
    }
//end:
    //free(shaped);
    return conver_str; 
...
wchar_t* convertHebrew(wchar_t* normal)
{
    ...
    for (i = 0; i < len; i++)
    {
        shaped[len - 1 - i] = normal[i];
    }
    shaped[len] = NIL;

    return (wchar_t*)shaped;
}
#endif
